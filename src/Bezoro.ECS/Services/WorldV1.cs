...
/// </summary>
public sealed partial class WorldV1 : IWorld, IDisposable, IAsyncDisposable
{
	...
	public WorldV1() : this(new WorldOptions()) { }

	public WorldV1(ReadOnlySpan<char> name) : this(name, new()) { }

	public WorldV1(WorldOptions options) : this(CreateDefaultName().AsSpan(), options) { }

	public WorldV1(ReadOnlySpan<char> name, WorldOptions options)
	{
		if (name.Trim().IsEmpty)
			throw new ArgumentException("WorldV1 name cannot be null or whitespace.", nameof(name));
	...
	public static WorldV1 Deserialize(byte[] bytes) =>
		WorldSerializer.Deserialize(bytes, SnapshotDeserializationOptions.Default);

	public static WorldV1 Deserialize(byte[] bytes, SnapshotDeserializationOptions options)
	{
		if (options is null) throw new ArgumentNullException(nameof(options));
	...
	/// </remarks>
	/// <returns>WorldV1 diagnostics snapshot.</returns>
	public WorldV1Diagnostics GetDiagnostics()
	{
		ThrowIfDisposed();
	...
	private static string CreateDefaultName() => $"WorldV1-{Guid.NewGuid():N}";
	...
	{
		if (_disposed)
			throw new ObjectDisposedException(nameof(WorldV1));
	}
	...
	private void ExecuteDisposeStep(Action<WorldV1> step, ref List<Exception>? exceptions)
	{
		try
		{
			step(this);
