...
internal sealed class Chunk
{
	...
	public void CopyValueTo(int sourceColumnIndex, int sourceRow, Chunk destination, int destinationColumnIndex, int destinationRow)
	{
		if (destination is null) throw new ArgumentNullException(nameof(destination));
		ValidateColumnIndex(sourceColumnIndex);
		destination.ValidateColumnIndex(destinationColumnIndex);
		Columns[sourceColumnIndex].CopyElementTo(sourceRow, destination.Columns[destinationColumnIndex], destinationRow);
	}
