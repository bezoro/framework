...
public static class ArrayHelper
{
	...
	/// <summary>
	///     Finds the first occurrence of <paramref name="value" /> in <paramref name="source" /> using parallel processing.
	/// </summary>
	/// <typeparam name="T">The type of elements in the array.</typeparam>
	/// <param name="source">The array to search within.</param>
	/// <param name="value">The value to find in the array.</param>
	/// <returns>An <see cref="ArrayElementInfo{T}" /> containing information about the found element or a not-found result.</returns>
	/// <remarks>
	///     - Returns <see cref="ArrayElementInfo{T}.NotFound(T,int)" /> when the array is empty
	///     - The search stops immediately after the first match is discovered
	///     - Uses parallel processing for arrays larger than <see cref="ParallelThreshold" />
	///     - Thread-safe implementation using lock mechanisms for concurrent access
	/// </remarks>
	public static ArrayElementInfo<T> FindElementInParallel<T>(T[] source, T value)
	{
		source.ThrowIfNull();

		if (source.Length == 0) return ArrayElementInfo<T>.NotFound(value, source.Length);
		...
		return index >= 0
				   ? new(index, value, source.Length)
				   : ArrayElementInfo<T>.NotFound(value, source.Length);
	}
	...
	/// <typeparam name="T">The type of elements in the array.</typeparam>
	/// <param name="source">The array to search within.</param>
	/// <param name="value">The element to find in the array.</param>
	/// <returns>
	...
	/// </remarks>
	public static ArrayElementInfo<T> FindElementSequentially<T>(T[] source, T value)
	{
		source.ThrowIfNull();
		...
		for (var i = 0; i < source.Length; i++)
		{
			if (comparer.Equals(source[i], value))
				return new(i, value, source.Length);
		}

		return ArrayElementInfo<T>.NotFound(value, source.Length);
	}

	/// <summary>
	///     Determines whether two one-dimensional arrays are equal.
	///     A <see cref="ArgumentNullException" /> is thrown when either argument is <c>null</c>.
	/// </summary>
	...
	public static bool SetupArrayWithFirstElement<T>(ref T[]? array, T element)
	{
		element.ThrowIfNull();
	...
	public static int FindNullIndex<T>(T[] array) where T : class
	{
		...
		bool useParallelSearch = array.Length > ParallelThreshold;
		var searchResult = useParallelSearch
							   ? FindElementInParallel(array, null)
							   : FindElementSequentially(array, null);
	...
	///     <list type="bullet">
	///         <item>if the item is null, it is represented as "null".</item>
	///         <item>if the item is a <see cref="string" />, it is enclosed in double quotes.</item>
	///         <item>otherwise, the item is represented by its result of calling <see cref="object.ToString()" />.</item>
	///     </list>
	/// </remarks>
	public static string GetElementName<T>(T item) =>
		item switch
		{
			null     => "null",
			string s => $"\"{s}\"",
	...
	public static void InitializeNullArray<T>(ref T[]? array, int count)
	{
		array = new T[count];
	}
	...
	public static void RemoveElement<T>(
		ref T[]         array,
		T               element,
		RemovalApproach removalApproach = RemovalApproach.Resize)
	{
		if (array == null || array.Length == 0) return;

		if (Equals(element, default(T))) return;

		// Decide on the removal method based on the size of the array
		if (array.Length > ParallelThreshold)
			RemoveElementParallel(ref array, element, removalApproach);
		else
			RemoveElementSequential(ref array, element, removalApproach);
	}

	/// <summary>
	///     Removes the first occurrence of <paramref name="item" /> from <paramref name="array" />
	///     and re-assigns the caller’s variable to the new array.
	/// </summary>
	/// <remarks>
	///     • Nothing happens when the array is <c>null</c>, empty, or the item is not found.
	///     • Works on all C# versions that support <c>ref</c> parameters (no “ref extension” syntax needed).
	/// </remarks>
	public static void RemoveInPlace<T>(ref T[] array, T item)
	{
		if (array is null || array.Length == 0) return;

		int idx = Array.IndexOf(array, item);

		if (idx < 0) return; // item not found; leave array untouched

		// Single-element array becomes empty
		if (array.Length == 1)
		{
			array = [];
			return;
		...
		if (idx > 0) Array.Copy(array, 0, result, 0, idx);

		// copy elements after the removed item
		int itemsAfter = array.Length - idx - 1;

		if (itemsAfter > 0) Array.Copy(array, idx + 1, result, idx, itemsAfter);

		array = result; // re-assign the caller’s reference
	}

	public static void ResizeByFactor<T>(ref T[] array, int factor)
	{
		array.ThrowIfNull();
		factor.ThrowIfLessThan(2);
	...
	private static void RemoveElementParallel<T>(
		ref T[]         array,
		T               element,
		RemovalApproach removalApproach
	)
	{
		int foundIndex = -1;
		...
		{
			case RemovalApproach.Resize:
				ResizeArrayParallel(ref array, foundIndex);
				break;

			case RemovalApproach.SetToNull:
				lock (lockObj)
				{
	...
	)
	{
		...
		Parallel.For(
			...
			i =>
			{
				...
				else if (i > index) tempArray[i - 1] = localArray[i];
			}
		);
...
public static class Merging
{
	...
	) where T : class
	{
		...
		if (elementsToAppendCount == 0) return;
	...
	) where T : class
	{
		...
		if (CheckForRemainingElements(source, sourceIndex, excludeNulls, out int appendCount)) return;
	...
	) where T : class
	{
			 ...
			 targetIndex++)
		{
			...
			while (sourceIndex < sourceLength)
			{
				...
				if (!excludeNulls || sourceElement != null) break;
			}

			// Assign to target only if a valid source element was found
			if (sourceElement != null || !excludeNulls) target[targetIndex] = sourceElement;
		}
	...
	{
		sourceArray.ThrowIfNull();
		targetArray.ThrowIfNull();
		elementsToAppend.ThrowIfLessThan(1);
		sourceStartIndex.ThrowIfLessThan(0);
