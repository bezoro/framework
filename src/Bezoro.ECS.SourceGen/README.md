...
- `SystemMetadataGenerator` infers read/write sets from ECS iteration and direct access calls (`World.Query(...).ForEach(...)`, generated `Run(...)` / `RunParallel(...)` job extensions, `World.Run(...)`, `QueryCursor.ForEach(...)`, `QueryCursor.Run(...)`, `Read/Write/TryWrite`, and explicit resource APIs), in addition to `[Reads]`/`[Writes]` attributes.
