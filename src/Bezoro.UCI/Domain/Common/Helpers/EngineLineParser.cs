...
internal static class UCIParser
{
	...
	{
		if (string.IsNullOrEmpty(line)) return new(line) { Type = EngineOutputType.Unknown };
		...
		return outputType switch
		{
			...
			UCIOutputType.UciOk or UCIOutputType.ReadyOk => new(line)
			{
				Type   = EngineOutputType.Status,
				Status = line.Trim()
			},
			_ => new(line) { Type = EngineOutputType.Unknown }
		};
	}
	...
	{
		if (string.IsNullOrEmpty(line)) return UCIOutputType.Unknown;
	...
	{
		var principalVariation = tokens.Skip(index + 1).ToList().AsReadOnly();
	...
	private static EngineOutput ParseBestMoveLine(string line)
	{
		...
		if (parts.Length < MinBestMoveTokens) return new(line) { Type = EngineOutputType.Unknown };
	...
	{
		if (!line.Contains("Checkers:")) return new(line) { Type = EngineOutputType.Unknown };

		string checkers = line.Split("Checkers:").Last().Trim();
		return new(line) { Checkers = checkers };
	}

	private static EngineOutput ParseIdLine(string line)
	{
		string[] parts = line.Split(' ', 3, StringSplitOptions.RemoveEmptyEntries);
		if (parts.Length < MinIdTokens) return new(line) { Type = EngineOutputType.Unknown };
	...
	{
		string[] tokens = line.AsSpan(InfoCommandLength).ToString().Split(' ', StringSplitOptions.RemoveEmptyEntries);
		...
		for (var i = 0; i < tokens.Length; i++) info = ProcessInfoToken(tokens, ref i, info);
	...
	private static EngineOutput ParseOptionLine(string line)
	{
		...
		if (tokens.Length < MinOptionTokens || !tokens[1].Equals("name", StringComparison.OrdinalIgnoreCase))
			return new(line) { Type = EngineOutputType.Unknown };
