...
{
	public static SearchResultOld ToSearchResult(this EngineOutput engineOutput)
	{
		...
		if (!engineOutput.AnalysisInfo.HasValue) return result;
		...
		else if (info.Mate.HasValue) result.FinalScore = info.Mate.Value;

		if (engineOutput.Checkers != null) result.Checkers = engineOutput.Checkers;
