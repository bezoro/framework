...
internal sealed class EngineProcessManager : IAsyncDisposable
{
	...
	public async Task StopAsync(CancellationToken cancellationToken = default)
	{
		...
		catch (Exception)
		{
			...
			if (!_engineProcess.HasExited) _engineProcess.Kill();
		}
	}
	...
	public async Task<string?> ReadLineAsync(CancellationToken ct, int timeoutMs = 5000)
	{
		...
		try
		{
			...
			if (completedTask != readTask) throw new TimeoutException("The engine response timed out.");
	...
	{
		if (_engineProcess == null) throw new UCIException("Engine process is null.");
	}
	...
	{
		if (_processOutput == null) throw new UCIException("Engine process output stream is null.");
	}
}
