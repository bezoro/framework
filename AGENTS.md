...
- Once a materially complex planning agent is dispatched, wait for its completed plan regardless of duration. Do not interrupt it, time-box it, or substitute local planning merely because it is slow; stop waiting only when the user changes direction or the agent returns a terminal failure.
