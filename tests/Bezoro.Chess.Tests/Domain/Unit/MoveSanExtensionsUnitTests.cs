...
[TestSubject(typeof(MoveSANExtensions))]
public class MoveSanExtensionsUnitTests
{
		...
		string    expectedSan)
	{
		...
		var capturedPiece = gameState.Board.GetPiece(toPos);
		var move          = Move.Capture(fromPos, toPos, piece, capturedPiece);
		...
		string    expectedSan)
	{
		...
		var toPos     = new Position(to);
		var piece     = new Piece(pieceType, gameState.ActiveColor);
		var move      = Move.Normal(fromPos, toPos, piece);
