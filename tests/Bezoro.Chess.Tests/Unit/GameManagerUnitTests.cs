...
public class GameManagerUnitTests
{
	...
	public void Forfeit_WhenGameIsAlreadyOver_DoesNothing()
	{
		...
		var gameManager = new GameManager();
		gameManager.Forfeit();
		var firstOutcome = gameManager.Outcome;
	...
	public void GetLegalMovesForPiece_ForStartingPawn_ShouldReturnTwoMoves()
	{
		...
		// Act
		var legalMoves = gameManager.GetLegalMovesForPiece(pawnPosition).ToList();
	...
	public void GetLegalMovesForPiece_ShouldOnlyReturnMovesThatDoNotLeaveKingInCheck()
	{
		...
		// Act
		var legalMoves = gameManager.GetLegalMovesForPiece(rookPosition).ToList();
	...
	public void GetLegalMovesForPiece_WhenPieceIsPinned_ShouldReturnNoMoves()
	{
		...
		// Act
		var legalMoves = gameManager.GetLegalMovesForPiece(knightPosition);
	...
	public void Redo_AfterNewMoveIsMade_ClearsRedoHistory()
	{
		...
		var piece1 = gameManager.CurrentState.GetPieceAt(from1);
		...
		var piece2 = gameManager.CurrentState.GetPieceAt(from2);
		...
		var piece3 = gameManager.CurrentState.GetPieceAt(from3);
		var move3  = Move.CreateNormal(from3, to3, piece3);
		gameManager.TryMakeMove(move3);

		// Act
		var result = gameManager.Redo();
	...
	public void Redo_AfterOneUndo_ReturnsTrueAndRestoresState()
	{
		...
		var piece       = gameManager.CurrentState.GetPieceAt(from);
		var move        = Move.CreateNormal(from, to, piece);
		gameManager.TryMakeMove(move);
		var stateAfterMove = gameManager.CurrentState;
		gameManager.Undo();
	...
	public void Undo_PastInitialState_ReturnsFalse()
	{
		...
		var finalUndoResult = gameManager.Undo(); // Attempt to undo past the start
