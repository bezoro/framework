using Bezoro.GameSystems.TimerSystem.Types;
using FluentAssertions;
using JetBrains.Annotations;
using Xunit;

namespace Bezoro.GameSystems.Tests.TimerSystem;

[TestSubject(typeof(TimerHandle))]
public class TimerHandleTests
{
	[Fact]
	public void None_ShouldHaveIdZero()
	{
		TimerHandle.None.Id.Should().Be(0);
	}

	[Fact]
	public void None_ShouldNotBeValid()
	{
		TimerHandle.None.IsValid.Should().BeFalse();
	}

	[Fact]
	public void WhenIdIsPositive_ShouldBeValid()
	{
		var handle = new TimerHandle(1);

		handle.IsValid.Should().BeTrue();
	}

	[Fact]
	public void WhenSameId_ShouldBeEqual()
	{
		var a = new TimerHandle(42);
		var b = new TimerHandle(42);

		a.Should().Be(b);
		(a == b).Should().BeTrue();
		(a != b).Should().BeFalse();
	}

	[Fact]
	public void WhenDifferentId_ShouldNotBeEqual()
	{
		var a = new TimerHandle(1);
		var b = new TimerHandle(2);

		a.Should().NotBe(b);
		(a == b).Should().BeFalse();
		(a != b).Should().BeTrue();
	}

	[Fact]
	public void WhenSameId_ShouldHaveSameHashCode()
	{
		var a = new TimerHandle(42);
		var b = new TimerHandle(42);

		a.GetHashCode().Should().Be(b.GetHashCode());
	}

	[Fact]
	public void ToString_ShouldContainId()
	{
		var handle = new TimerHandle(7);

		handle.ToString().Should().Be("Timer(7)");
	}

	[Fact]
	public void WhenComparedToObject_ShouldHandleCorrectly()
	{
		var handle = new TimerHandle(1);

		handle.Equals((object)new TimerHandle(1)).Should().BeTrue();
		handle.Equals((object)new TimerHandle(2)).Should().BeFalse();
		handle.Equals("not a handle").Should().BeFalse();
	}

	[Fact]
	public void Default_ShouldEqualNone()
	{
		TimerHandle handle = default;

		handle.Should().Be(TimerHandle.None);
	}
}
