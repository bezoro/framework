...
public sealed class LoggerStageTests
{
	...
	{
		var lastStageField = typeof(Logger).GetField("_lastStage", BindingFlags.NonPublic | BindingFlags.Static)
			?? throw new InvalidOperationException("Logger stage state field was not found.");
		var previousStage = lastStageField.GetValue(null);
		lastStageField.SetValue(null, null);
		...
		{
			lastStageField.SetValue(null, previousStage);
		}
	}
}
